...
import pandas as pd
import numpy as np
from scorescanner.utils.statistical_metrics import (calculate_js_distances, one_vs_rest_woe, cramers_v)
import plotly.graph_objects as go
from plotly.subplots import make_subplots


def bins_stats(df: pd.DataFrame, feature: str, target_var: str, cat_ref=None) -> pd.DataFrame:
    """
    ...
    for col in bins_count.columns:
        stats[f'percent_{col.split("_")[1]}'] = (stats[col] / stats['count'] * 100).fillna(0)
    ...
    stats_js = stats.merge(js_distances, left_on=feature, right_on='Category', how='left')
    # Merging bins stats with WoE
    final_stats = stats_js.merge(woe_df, left_on=feature, right_on='Category', how='left')
    # Extracting upper bounds from intervals
    final_stats['max_of_interval'] = final_stats[feature].apply(lambda x: float(x.split(',')[1].strip(')').strip(']')) if ',' in x else np.nan)

    return final_stats.sort_values(by='max_of_interval').drop(columns=["max_of_interval","Category_x", "Category_y"])


def generate_bar_plot(df: pd.DataFrame, feature: str, target_var: str, cat_ref=None, colors=['#82E0AA', '#EC7063', '#5DADE2', '#F7DC6F', '#FDEDEC', '#F4D03F']):
    """
    ...
    info_value = one_vs_rest_woe(df=df, feature=feature, target_var=target_var,cat_ref=cat_ref)[1]
    ...
    for count_col, percent_col, color in zip(count_cols, percent_cols, colors):
        ...
        fig.add_trace(
            go.Bar(
                ...
                hoverinfo='all',
                hovertemplate=stats_df.apply(lambda row: f"<b>{count_col}: {row[count_col]}<br>"
                                                  f"Percentage: {row[percent_col]:.2f}%<br>"
                                                  f"WoE: {row['WoE']:.2f}<br>"
                                                  f"JS Distance: {row['Jensen-Shannon Distance']:.2f}", axis=1),
                text=stats_df.apply(lambda row: f"<b>{count_col}: {row[count_col]}<br>"
                                                  f"Percentage: {row[percent_col]:.2f}%<br>", axis=1),
...
def plot_woe(df: pd.DataFrame, feature: str, target_var: str, cat_ref=None, colors=['#9B59B6']):
    """
    ...
    final_stats = woe_df[~woe_df['Category'].isin(['Special', 'Missing'])].sort_values(by='Category')
    ...
    fig.add_trace(go.Bar(
        ...
        hovertext=woe_df.apply(lambda row: f"Category: {row['Category']}<br>WoE: {row['WoE']:.2f}", axis=1)
    ))
...
def plot_js(df: pd.DataFrame, feature: str, target_var: str, colors=['#F7DC6F']):
    ...
    final_stats = js_df[~js_df['Category'].isin(['Special', 'Missing'])].sort_values(by='Category')
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(color=colors[0]),
        hoverinfo='text',  # Use text for hover information
        text=js_df.apply(lambda row: f"<b>JS: {row['Jensen-Shannon Distance']:.2f}", axis=1),
        hovertext=js_df.apply(lambda row: f"<b>Category: {row['Category']}<br>JS: {row['Jensen-Shannon Distance']:.2f}", axis=1)
    ))
