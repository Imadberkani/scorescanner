...
import pandas as pd
import numpy as np
from scorescanner.preprocessing import (outlierdetector, multioptbinning, logisticregressionpreparer)
...
class testoutlierdetector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data = {
            'col1': np.concatenate([np.random.normal(0, 1, 95), np.random.normal(10, 5, 5)]),
            'col2': np.concatenate([np.random.normal(0, 1, 95), np.random.normal(20, 5, 5)])
        }
    ...
    def test_iqr_constant(self):
        ...
        detector = outlierdetector(columns=self.columns, method='IQR', replacement_method='constant', replacement_value=-999)
        ...
        outliers_mask = {col: self.df_test[col].apply(lambda x: detector._is_outlier(x, col)) for col in self.columns}
        ...
        for col in self.columns:
            ...
            for i in outliers_mask[col][outliers_mask[col]].index:
                ...
                self.assertEqual(df_transformed.loc[i, col], -999, f"Outlier not replaced in column {col}")
                
                # Verifying that outliers have been correctly identified.
                outlier_value = self.df_test.loc[i, col]
                lower_bound, upper_bound = detector.bounds[col]['lower'], detector.bounds[col]['upper']
                self.assertTrue(outlier_value < lower_bound or outlier_value > upper_bound, f"Identified outlier {outlier_value} in column {col} is within bounds")
    ...
    def test_zscore_constant(self):
        ...
        detector = outlierdetector(columns=self.columns, method='z-score', replacement_method='constant', replacement_value=-999, z_threshold=3)
        ...
        for col in self.columns:
            ...
            for i in outliers_mask[col][outliers_mask[col]].index:
                ...
                # Verifying that outliers have been correctly identified.
                mean, std_dev = detector.stats[col]['mean'], detector.stats[col]['std_dev']
                z_score = abs((self.df_test.loc[i, col] - mean) / std_dev)
                self.assertTrue(z_score > detector.z_threshold, f"Identified outlier {self.df_test.loc[i, col]} in column {col} has Z-score {z_score} within threshold")
    ...
    def test_iqr_mean(self):
        ...
        detector = outlierdetector(columns=self.columns, method='IQR', replacement_method='mean')
        ...
        for col in self.columns:
            ...
            for i in outliers_mask[col][outliers_mask[col]].index:
                ...
                self.assertAlmostEqual(df_transformed.loc[i, col], mean_non_outliers, msg=f"Outlier in column {col} not replaced by mean")
    ...
    def test_iqr_capping_flooring(self):
        ...
        detector = outlierdetector(columns=self.columns, method='IQR', replacement_method='capping_flooring')
        ...
        for col in self.columns:
            ...
            for i in outliers_mask[col][outliers_mask[col]].index:
                ...
                self.assertTrue(lower_bound <= transformed_value <= upper_bound, f"Outlier in column {col} not correctly capped/floored")
                
                # Verifying that outliers have been correctly identified
                outlier_value = self.df_test.loc[i, col]
                self.assertTrue(outlier_value < lower_bound or outlier_value > upper_bound, f"Identified outlier {outlier_value} in column {col} is within bounds before transformation")
...
class TestMultiOptBinning(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.df_test['binary_target'] = (4*self.df_test['col1'] - 3*self.df_test['col1']  > 0).astype(int)
        ...
        self.df_test['multiclass_target'] = np.where(combined_feature <= first_tertile, 0,
                                                     np.where(combined_feature <= second_tertile, 1, 2))

        ### Creating a continuous target variable that has a relationship with features
        self.df_test['continuous_target'] = 0.7 * self.df_test['col1'] + 0.3 * self.df_test['col2'] + np.random.normal(0, 1, n_samples)
    ...
    def test_binary_target_binning(self):
        ...
        for variable in self.binner.variables:
            ...
            self.assertTrue(transformed_df[variable].dtype.name in ['category','object'],
                        f"The transformed data for {variable} is not of type 'category', indicating it may not be properly binned.")
            self.assertTrue(transformed_unique_values >= self.binner.min_n_bins,
...
class TestLogisticRegressionPreparer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.preparer = logisticregressionpreparer(columns=columns_to_encode, column_dict=column_dict)
    ...
    def test_one_hot_encoding_transformation(self):
        ...
        for col in self.preparer.columns:
            self.assertNotIn(col, transformed_df.columns, f"Column {col} was not removed after one-hot encoding.")
        ...
        self.assertEqual(len(transformed_df.columns), expected_columns,
